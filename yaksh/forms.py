...
from yaksh.models import (
    get_model_class, Profile, Guardian, Quiz, Question, Course, QuestionPaper, Lesson,
...
class GuardianRegisterForm(forms.Form):
    ...
    def clean_user(self):
        ...
        try:
            ...
            return u_name
    """
    def clean_child_user(self):
        u_name = self.cleaned_data["child_user"]
        if u_name.strip(UNAME_CHARS):
            msg = "Only letters, digits, period and underscore characters are"\
                  " allowed in child username"
            raise forms.ValidationError(msg)

        child_user = None
        if User.objects.filter(username=u_name).exists():
            child_user = User.objects.get(username=u_name)
        if not User.objects.filter(username=u_name).exists():
            raise forms.ValidationError("Children Username does not exists")
        elif Parent.objects.filter(child_user_id=child_user.id).exists():
            print(Parent.objects.filter(child_user_id=child_user.id),"ashish")
            raise forms.ValidationError("Children already has a parent")
        return u_name
    """
    ...
    def save(self):
        ...
        #child_user = self.cleaned_data["child_user"]
        #child_user = child_user.lower()
        pwd = self.cleaned_data["password"]
        email = self.cleaned_data['email']
        new_user = User.objects.create_user(username, email, pwd)
        #print(new_user,"ashish")
        #if User.objects.get(username=child_user):
        #    print(User.objects.get(username=child_user))
        new_user.first_name = self.cleaned_data["first_name"]
        new_user.last_name = self.cleaned_data["last_name"]
        new_user.save()

        cleaned_data = self.cleaned_data
        parent = Guardian(user=User.objects.get(username=username))
        parent.user = User.objects.get(username=username)
        #parent.is_parent = False 
        parent.is_email_verified = False 
        parent.save()

        return username , pwd, email

#########################################################################################
class HodRegisterForm(forms.Form):
    """form for HODs registration"""
    ...
    staff_number = forms.CharField(
        ...
        widget=forms.TextInput(
            {'class': form_input_class, 'placeholder': "Staff Number"}
        ))
    ...
    def save(self):
        ...
        new_profile.language = cleaned_data["language"]
        new_profile.position = cleaned_data["position"]
        new_profile.timezone = cleaned_data["timezone"]
        
        new_profile.is_email_verified = False # email verification
        new_profile.is_hod = False #hod verification
        new_profile.save()
...
class TeacherRegisterForm(forms.Form):
    """A Class to create new form for Teacher's Registration.
    It has the various fields and functions required to register
    a new user to the system"""
    ...
    def save(self):
        ...
        cleaned_data = self.cleaned_data
        new_profile = Profile(user=new_user)
        new_profile.staff_number = cleaned_data["staff_number"]
        new_profile.institute = cleaned_data["institute"]
        new_profile.department = cleaned_data["department"]
        new_profile.position = cleaned_data["position"]
        new_profile.timezone = cleaned_data["timezone"]
...
class QuestionFilterForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if lang != None:
            points = Question.objects.filter(language=lang).values_list('points', flat=True).distinct()
        else:
            points = Question.objects.filter(user=user).values_list('points', flat=True).distinct()
