...
class Profile(models.Model):
    ...
    language = models.CharField(max_length=24, choices=languages,blank=True, null=True)
...
class Guardian(models.Model):
    """Profile for Parents which has many to one relation with Children"""
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    is_email_verified = models.BooleanField(default=False)
...
class Ward(models.Model):
    ...
    guardian_user = models.ForeignKey(to=Guardian, on_delete=models.CASCADE)
...
class Attendance(models.Model):
    ...
    date = models.DateField(default=datetime.now(pytz.timezone('Asia/Kolkata')))
    start_time = models.TimeField(default=datetime.now(pytz.timezone('Asia/Kolkata')))
    ...
    """calculate day and total days"""
    def dayanddays(month, year, date):
        month_code = {'jan':11, 'feb':12, 'mar':1, 'apr':2, 'may':3, 'jun':4, 'jul':5, 'aug':6, 'sep':7, 'oct':8, 'nov':9, 'dec':10}
        ...
        return day%7,total_days

    
class AttendanceCourse(models.Model):
    course = models.ForeignKey(to=Course, on_delete=models.CASCADE)
    attendance = models.ForeignKey(to=Attendance, on_delete=models.CASCADE)
...
class Question(models.Model):
    ...
    def _add_yaml_to_zip(self, zip_file, q_dict, path_to_file=None):
        ...
        for elem in q_dict:
            ...
            relevant_dict['negative_marks'] = elem.pop('negative_marks')
            ...
            data.update(
                {
                    ...
                    'negative_marks': content.negative_marks,
                    'testcase': tc_data,
                }
            )
