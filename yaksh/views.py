...
from yaksh.models import (
    ...
    QRcodeHandler, dict_to_yaml, Attendance, AttendanceCourse
)
from stats.models import TrackLesson
from yaksh.forms import (
    UserRegisterForm, HodRegisterForm,TeacherRegisterForm,GuardianRegisterForm, UserLoginForm, QuizForm, QuestionForm,
...
from notifications_plugin.models import Notification, NotificationMessage
import hashlib
from yaksh.models import Guardian, Ward
...
################################################################################################################
def is_moderator(user, group="moderator"):
...
################################################################################################################
def is_hod(user, group="HOD"):
    """if user is in HOD group than wheaher user is having HOD rights or not"""
    if user.groups.filter(name=group).exists():
...
##########################################################################################################################
def is_guardian(user, group="guardian"):
    """check if user is having parent rights"""
    if user.groups.filter(name=group).exists():
        return True
    return False 

##########################################################################################################################
def is_student(user,group="student"):
...
######################################################################################################################################
# for testing purpose and not for development
def testing(request):
    

    return render(request,'yaksh/test.html')
...
def guardian_register(request):
    ...
    user = request.user 
    """All the redirecting part if the user is logged in and tries to register"""
    index(request)
    
    """taking inputs from forms"""
    if request.method == "POST":
        form = GuardianRegisterForm(request.POST)
        if form.is_valid():
            uname, pwd, email = form.save()

            """Creating a new group not exists"""
            newGroup = Group()
            if len(Group.objects.filter(name="guardian")) == 0:
                newGroup.name = "guardian" 
                newGroup.save()

            group = Group.objects.get(name="guardian")
            users = User.objects.get(username=uname)
            users.groups.add(group) #adding user to parent group

            """ creating a 'hod' notification for admin using notification_plugin """
            summary = "guardian"
            description = "new guardian account is created"
            msg_type = "info"
            receiver = User.objects.filter(is_staff=True, is_active=True, is_superuser=True) # receivers who are admin
            
            receiver_id = []
            for r in receiver:
                receiver_id.append(r.pk)

            mssg = NotificationMessage.objects.add_single_message(users.pk, summary, description, msg_type)  # creating a notification message
            Notification.objects.add_bulk_user_notifications(receiver_id, mssg.id) # linking above message with the admins
            ...
            return my_render_to_response(
                request, 'yaksh/guardian_register.html', {'form': form}
            )
...
def hod_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            u_name, pwd, user_email, key = form.save() # user is successfully added to the database

            #creating a HOD group if not exists
            newGroup = Group()
            if len(Group.objects.filter(name="HOD"))==0:
                newGroup.name ="HOD"
                newGroup.save()

            group = Group.objects.get(name="HOD")
            users = User.objects.get(username=u_name)
            users.groups.add(group) #adding user to HOD group

            # creating a 'hod' notification for admin using notification_plugin
            summary = "HOD"
            description = "new HOD account is created, verify account in Profile section to make it as a HOD"
            msg_type = "info"
            receiver = User.objects.filter(is_staff=True, is_active=True, is_superuser=True) # receivers who are admin
            ...
            message = {'message': "Wait for Admin to authorize your account"}
            return my_render_to_response(request, 'yaksh/not_authorized.html', message)
            
        else:
            return my_render_to_response(
                request, 'yaksh/hod_register.html', {'form': form}
            )
    else:
...
def teacher_register(request):
    ...
    if user.is_authenticated:
        ...
        message = {'message': "You are not yet authorized by admin"}
        return my_render_to_response(request, 'yaksh/not_authorized.html', message)
    context = {}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            u_name, pwd, user_email, key = form.save() # user is succesfully added to the database
            
            #creating a moderator group if not exists
            newGroup = Group()
            if len(Group.objects.filter(name="moderator"))==0:
                newGroup.name ="moderator"
                newGroup.save()

            group = Group.objects.get(name="moderator")
            users = User.objects.get(username=u_name)
            users.groups.add(group) #adding user to moderator group

            # creating a 'teacher' notification for admin in notification_plugin
            summary = "teacher"
            description = "new teacher account is created, verify account in profile section to make it as a moderator"
            msg_type = "info"
            receiver = User.objects.filter(is_staff=True, is_active=True, is_superuser=True) # receivers who are admin
            ...
            return my_render_to_response(
                request, 'yaksh/teacher_register.html', {'form': form}
            )
    else:
...
def user_register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #creating a student group if not exists
            newGroup = Group()
            if len(Group.objects.filter(name="student"))==0:
                newGroup.name ="student"
                newGroup.save()

            # adding user to student group
            group = Group.objects.get(name="student")
            users = User.objects.get(username=u_name)
            users.groups.add(group)

            # creating a 'student' notification for admin in notification_plugin
            summary = "student"
            description = "new student account is created"
            msg_type = "info"
            receiver = User.objects.filter(is_staff=True, is_active=True, is_superuser=True) # receivers who are admin
...
def add_question(request, question_id=None):
    ...
    if request.method == 'POST':
        ...
        if is_hod(user) and (request.POST['language'] != user.profile.language):
    ...
    context['group']=group
    context['language'] = user.profile.language
...
def user_login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user.groups.filter(name="moderator").exists() or user.groups.filter(name="HOD").exists():
                if not (is_moderator(user) or is_hod(user)):
                    message = {'message': "You are not yet authorized by admin"}
                    return my_render_to_response(request, 'yaksh/not_authorized.html', message)
...
def guardian_dashboard(request):
    ...
    """to add ward with the guardian"""
    wardAddMssg = ""
    ward_user = None
    if 'wardAddButton' in request.POST:
        warduser = request.POST["wardAdd"]
        if not User.objects.filter(username=warduser).exists():
            wardAddMssg = "username does not exists"
        elif Ward.objects.filter(ward_user=warduser).exists():
            wardAddMssg = "student already has a parent"
        elif not is_student(User.objects.get(username=warduser)):
            wardAddMssg = "username is not a student"
        else:
            Ward(ward_user=warduser, guardian_user = Guardian.objects.get(user=user)).save()
            wardAddMssg = "Ward Successfully Added"

    if 'ward_selected' in request.POST:
        ward_selected = request.POST["ward_selected"]  
        ward_user = User.objects.get(username=ward_selected)
        course_enrolled = []
        course_completed = {}
        quiz_marks = {}
        ...
        for course in Course.objects.filter(students=User.objects.get(username=ward_selected).id):
            ...
            #print(course.get_completion_percent(ward_user))
            course_completed[course.id] = course.get_completion_percent(ward_user)
            print(course.id)
            """for each quiz/unit"""
            for unit in course.get_learning_units():
                print(unit, unit.id)
                quiz_dict = {}
                quiz_question_id = QuestionPaper.objects.get(quiz=unit.id).id
                quiz_dict["total_marks"] = QuestionPaper.objects.get(quiz=unit.id).total_marks
                quiz_marks_obtained = {}
                """for each answer per quiz"""
                for answer in AnswerPaper.objects.filter(user=ward_user, course=course.id, question_paper=quiz_question_id):
                    marks_obtained_per_attempt = {}
                    marks_obtained_per_attempt['marks'] = answer.marks_obtained
                    marks_obtained_per_attempt['percent'] = answer.percent
                    marks_obtained_per_attempt['passed'] = answer.passed
                    marks_obtained_per_attempt['status'] = answer.status
                    quiz_marks_obtained[answer.attempt_number] = marks_obtained_per_attempt
                quiz_dict["marks_obtained"] = quiz_marks_obtained

                quiz_marks[unit.id] = quiz_dict
                #print("ash", quiz_dict)
        print(quiz_marks)
        ...
        context['learning_module'] = LearningModule
        context['course_completed'] = course_completed

    if 'attendance_button' in request.POST:
        context['attendance_ward'] = request.POST['attendance_button']

    context["wardAddMssg"] = wardAddMssg
    context["ward_selected"] = ward_user
    
    guardian_id = Guardian.objects.get(user_id=user.id)
    ward = Ward.objects.filter(guardian_user_id=guardian_id)

     
    
    context["wards"] = ward
    return render(request, "yaksh/guardian_dashboard.html", context)

#######################################################################################################################################################
def guardian_calender(request):
    month_name = {1:'jan',2:'feb', 3:'mar', 4:'apr', 5:'may', 6:'jun', 7:'jul', 8:'aug', 9:'sep', 10:'oct', 11:'nov', 12:'dec'}
    context={}
    ward_user = None
    curr_date = date.today()
    month = month_name[curr_date.month]
    year = curr_date.year

    if 'attendance_button' in request.GET:
        ward_user = request.GET['attendance_button']

    if request.is_ajax and request.method == 'POST':
        present_days = []
        month_code = {'jan':1, 'feb':2, 'mar':3, 'apr':4, 'may':5, 'jun':6, 'jul':7, 'aug':8, 'sep':9, 'oct':10, 'nov':11, 'dec':12}
        month = month_code[request.POST['month']]
        year = request.POST['year']
        user = request.POST['user']
        #print(month, year)

        for data in Attendance.objects.filter(user = User.objects.get(username = user)):
            #print(AttendanceCourse.objects.filter(attendance = data))
            if int(month)==data.date.month and int(year)==data.date.year:
                present_days.append(data.date.day)
        
        #print(present_days)
        return JsonResponse({"present_days":present_days})

    context = {
        'month' : month,
        'year' : year,
        'ward_user' : ward_user, 
        'year_list' : [i for i in range(2000,2101)],
    }
    return render(request, 'yaksh/calender.html',context)
...
def start(request, questionpaper_id=None, attempt_num=None, course_id=None,
    ...
    if not course.is_enrolled(user):
        msg = 'You are not enrolled in {0} course'.format(course.name)
        if (is_moderator(user) or is_hod(user)) and course.is_trial:
            return prof_manage(request, msg=msg)
        return quizlist_user(request, msg=msg)
...
def _get_questions(user, question_type, marks):
    ...
    if question_type:
        if is_hod(user):
            questions = Question.objects.filter(type=question_type,language=user.profile.language,active=True)
        else:
...
def show_all_questions(request):
    ...
    if is_moderator(user):
        questions = Question.objects.get_queryset().filter(user_id=user.id, active=True).order_by('-id')
    elif is_hod(user):
...
def user_data(request, user_id, questionpaper_id=None, course_id=None):
    ...
    if not (is_moderator(current_user) or is_hod(current_user)):
        raise Http404('You are not allowed to view this page!')
    user = User.objects.get(id=user_id)
...
def download_quiz_csv(request, course_id, quiz_id):
    current_user = request.user
    if not (is_moderator(current_user) or is_hod(user)):
        raise Http404('You are not allowed to view this page!')
    course = get_object_or_404(Course, id=course_id)
...
def remove_teachers(request, course_id):
    ...
    user = request.user
    course = get_object_or_404(Course, pk=course_id)
    if not (is_moderator(user) or is_hod(user)) and (not course.is_creator(user) and
...
def regrade(request, course_id, questionpaper_id, question_id=None,
    ...
    user = request.user
    course = get_object_or_404(Course, pk=course_id)
    if not (is_moderator(user) or is_hod(user)) or (course.is_creator(user) and
                                  course.is_teacher(user)):
        raise Http404('You are not allowed to view this page!')
...
def course_attendance(request, course_id):
    ...
    if request.method == 'POST' and request.POST['action'] == 'get_enrolled_user':
        enrolled = course.get_enrolled().values()
        ...
        for enroll in enrolled:
            user_id.append(enroll['id'])
            first_name.append(enroll['first_name'])
            last_name.append(enroll['last_name'])

            user_attendance[enroll['id']] = []
            for data in Attendance.objects.filter(user_id=enroll['id']):
                if data.date.month==int(month) and data.date.year==int(year) and AttendanceCourse.objects.filter(attendance_id=data.id, course_id=course_id).exists():
                    user_attendance[enroll['id']].append(data.date.day)
                    
        # print(user_attendance)
        return JsonResponse({ "user_id" : user_id, 'first_name':first_name, 'last_name':last_name, 'user_attendance':user_attendance })

    if request.method == 'POST' and request.POST['action'] == 'save_data':
        month = month_code[request.POST['month']]
        year = request.POST['year']
        checkbox_data = json.loads(request.POST['checkbox_data'])
        print(checkbox_data, month)
        
        for u_id in checkbox_data:
            present_before = set()
            for a in Attendance.objects.filter(user_id = int(u_id)):
                if AttendanceCourse.objects.filter(attendance_id=a.id, course_id = course_id).exists():
                    present_before.add(a.date.day)
            print(present_before, 'ash')

            present_now = set(checkbox_data[u_id])
            print(present_now, 'sin')

            """unchecked i.e. present before but not now"""
            for ele in present_before:
                if ele not in present_now:
                    temp_date = datetime.datetime(int(year), int(month), ele).strftime("%Y-%m-%d")
                    temp1 = Attendance.objects.get(date=temp_date, user_id=int(u_id))
                    temp2 = AttendanceCourse.objects.get(attendance_id=temp1.id, course_id=course_id)
                    if temp1 and temp2:
                        temp2.delete()


            """checked i.e. not present before but present now"""
            for ele in present_now:
                if ele not in present_before:
                    temp_date = datetime.datetime(int(year), int(month), ele).strftime("%Y-%m-%d")
                    if not Attendance.objects.filter(user_id=int(u_id), date=temp_date).exists():
                        temp1 = Attendance(user_id=int(u_id), group='student', date=temp_date)
                        temp1.save()
                    attendance_id = Attendance.objects.get(user_id=int(u_id), date=temp_date).id
                    if not AttendanceCourse.objects.filter(attendance_id=attendance_id, course_id=course_id).exists():
                        temp2 = AttendanceCourse(attendance_id=attendance_id, course_id=course_id)
                        temp2.save()
    ...
    return my_render_to_response(request, 'yaksh/course_attendance.html', context)
